...
// Assuming a dictionary {label->value}
// where the item would be the key (firstname, lastname)
// and the value is the count of ocurrencies
const getRanked = (dict, max = DEFAULT_RANKING_COUNT) =>
  Object.entries(dict).sort((a, b) =>
    b[1]-a[1]).slice(0, max);

// Take the first N names from the array
// where first and last are unique for that property
const getModifiedNames = (array, max = DEFAULT_MODIFIED_NAMES_COUNT) => {
  // I would have used a reduce here,
  // but reduce cannot be early breaked
  // so if I already found the N modified names that I want,
...
// Because I know there wont be firstNames and lastNames dups,
// I can offset 1 key of first or last and it will do the job
